...
|----------------------|---------------------|
| `next`               | フレームワーク       |
| `drizzle-orm` + `better-sqlite3` + `drizzle-kit` | ORM + SQLite |
...
- SQLiteファイルはプロジェクトルートの `data/morningbrief.db`

### 環境変数

環境変数は不要。SQLiteデータベースのパスは `data/morningbrief.db` にハードコードされており、better-sqlite3が直接アクセスする。
